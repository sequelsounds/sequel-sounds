...
Writes, into supabase/functions/composition-licence/:
  licence-template.pdf   the template with every placeholder REMOVED
  layout.ts              where each value goes (generated; do not hand-edit)
...
def main():
    ...
    dst = os.path.join(OUT, 'licence-template.pdf')
    pdf.save(dst)
    ...
    with open(os.path.join(OUT, 'layout.ts'), 'w') as f:
        ...
        f.write('export const LAYOUT = ' + json.dumps(layout, indent=2) + ' as const\n')
