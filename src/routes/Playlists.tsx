...
import { usePlaylistActions, usePlaylists, type PlaylistSummary } from '../lib/queries'
...
export default function Playlists() {
  ...
  for (const p of playlists.data ?? []) {
    ...
    if (!groups.has(key)) {
      groups.set(key, {
        title: p.projects_mirror ? splitProjectName(p.projects_mirror.name).title : 'Not attached to a project',
        rows: [],
      })
    }
  ...
  return (
    <>
      <div className="header-band">
        <div className="flex items-center justify-between gap-4">
          ...
          <button
            ...
            onClick={async () => {
              const id = await actions.createPlaylist.mutateAsync({ projectId: null })
              creator.open(id)
            }}
          >
            New playlist
      ...
      <div className="min-h-0 flex-1 overflow-auto">
        {playlists.isPending && <p className="px-7 py-4 text-sequel-mid">Loading…</p>}
        {playlists.error && <p className="form-error px-7 py-4">{playlists.error.message}</p>}
        {playlists.data && playlists.data.length === 0 && (
          <p className="px-7 py-6 text-sequel-mid">No playlists yet.</p>
        )}
        {ordered.map(([key, group]) => (
          <section key={key}>
            <h2 className="submission-title sentence-case font-sans px-7 pb-2 pt-4">{group.title}</h2>
            <PlaylistRows rows={group.rows} onOpen={openPlaylist} current={creator.playlistId} />
          </section>
        ))}
      </div>
    </>
        ...
        {rows.map((p) => {
          ...
          return (
            ...
            >
              ...
              <td className="secondary">{n > 0 ? plural(n, 'track') : 'empty'}</td>
...
export function PlaylistRoute() {
  ...
  useEffect(() => {
    ...
    creator.open(id)
    navigate(p?.project_id ? `/projects/${p.project_id}?tab=playlists` : '/playlists', { replace: true })
