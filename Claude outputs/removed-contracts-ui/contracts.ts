...
export type Renewal = {
  ...
  brand: string | null
  /** Expiring: the EARLIEST expiry on the project. Expired: the LATEST, which
   *  is when cover actually stopped. The rest travel with it either way. */
  end_date: string
  days_left: number
  /** Expired only: how long ago it lapsed. */
  days_since?: number
...
export function useRenewals() {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await rpc('track_contract_renewals')
      if (error) throw error
      return data as {
        due: Renewal[]
        expired: Renewal[]
        unresolved: Unresolved[]
        /** Not a list: perpetual grants are never chased, and the count is
         *  only shown so their absence is explained. */
        perpetual: number
      }
    },
  })
